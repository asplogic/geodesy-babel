...
class LatLonEllipsoidal {
  ...
  static parse(...args) {
    ...
    let lat = undefined,
        lon = undefined,
        height = undefined; // single { lat, lon } object
...
class Cartesian extends Vector3d {
  ...
  toLatLon(ellipsoid = ellipsoids.WGS84) {
    ...
    const cosβ = sinβ / tanβ; // geodetic latitude (Bowring eqn.18: tanφ = z+ε²⋅b⋅sin³β / p−e²⋅cos³β)

    const φ = isNaN(cosβ) ? 0 : Math.atan2(z + ε2 * b * sinβ * sinβ * sinβ, p - e2 * a * cosβ * cosβ * cosβ); // longitude

    const λ = Math.atan2(y, x); // height above ellipsoid (Bowring eqn.7)
