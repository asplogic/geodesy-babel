...
class LatLonNvectorSpherical {
  ...
  toNvector() {
    ...
    const sinλ = Math.sin(λ),
          cosλ = Math.cos(λ); // right-handed vector: x -> 0°E,0°N; y -> 90°E,0°N, z -> 90°N
  ...
  finalBearingTo(point) {
    if (!(point instanceof LatLonNvectorSpherical)) throw new TypeError(`invalid point ‘${point}’`); // get initial bearing from destination point to this point & reverse it by adding 180°
  ...
  intermediatePointTo(point, fraction) {
    if (!(point instanceof LatLonNvectorSpherical)) throw new TypeError(`invalid point ‘${point}’`);
    if (isNaN(fraction)) throw new TypeError(`invalid fraction ‘${fraction}’`); // angular distance between points; tanδ = |n₁×n₂| / n₁⋅n₂

    const n1 = this.toNvector();
    const n2 = point.toNvector();
    const sinθ = n1.cross(n2).length;
    const cosθ = n1.dot(n2);
    const δ = Math.atan2(sinθ, cosθ); // interpolated angular distance on straight line between points

    const δi = δ * Number(fraction);
    const sinδi = Math.sin(δi);
    const cosδi = Math.cos(δi); // direction vector (perpendicular to n1 in plane of n2)
  ...
  static intersection(path1start, path1brngEnd, path2start, path2brngEnd) {
    ...
    let c1 = null,
        c2 = null,
        path1def = null,
        path2def = null; // c1 & c2 are vectors defining great circles through start & end points; p × c gives initial bearing vector
    ...
    const i2 = c2.cross(c1); // TODO am I making heavy weather of this? is there a simpler way to do it?
  ...
  isWithinExtent(point1, point2) {
    ...
    const n0 = this.toNvector(),
          n1 = point1.toNvector(),
          n2 = point2.toNvector(); // n-vectors
    // get vectors representing p0->p1, p0->p2, p1->p2, p2->p1

    const δ10 = n0.minus(n1),
          δ12 = n2.minus(n1);
    const δ20 = n0.minus(n2),
          δ21 = n1.minus(n2); // dot product δ10⋅δ12 tells us if p0 is on p2 side of p1, similarly for δ20⋅δ21
  ...
  static trilaterate(point1, distance1, point2, distance2, point3, distance3, radius = 6371e3) {
    ...
    const n3 = point3.toNvector(),
          δ3 = Number(distance3) / Number(radius); // the following uses x,y coordinate system with origin at n1, x axis n1->n2
  ...
  isEnclosedBy(polygon) {
    ...
    const p = this.toNvector(); // get vectors from p to each vertex

    const vectorToVertex = [];

    for (let v = 0; v < nVertices; v++) vectorToVertex[v] = p.minus(polygon[v].toNvector());

    vectorToVertex.push(vectorToVertex[0]); // sum subtended angles of each edge (using vector p to determine sign)
  ...
  static areaOf(polygon, radius = 6371e3) {
    const R = Number(radius); // close the polygon so that the last point equals the first point
    ...
    c.push(c[0]); // sum interior angles; depending on whether polygon is cw or ccw, angle between edges is
    ...
    const Σθ = n * π - Math.abs(Σα); // note: angle between two sides of a spherical triangle is acos(c₁·c₂) where cₙ is the
