...
import txParams from './latlon-ellipsoidal-referenceframe-txparams.js'; // freeze static properties
...
Object.keys(txParams).forEach(tx => {
  Object.freeze(txParams[tx]);
  Object.freeze(txParams[tx].params);
  Object.freeze(txParams[tx].rates);
});
...
class LatLonEllipsoidal_ReferenceFrame extends LatLonEllipsoidal {
  ...
  static parse(...args) {
    ...
    if (!isNaN(args[1]) && typeof args[2] == 'object') {
      // latlon, height, referenceFrame, [epoch]
      [referenceFrame] = args.splice(2, 1);
      [epoch] = args.splice(2, 1);
    }

    if (!isNaN(args[2]) && typeof args[3] == 'object') {
      // lat, lon, height, referenceFrame, [epoch]
      [referenceFrame] = args.splice(3, 1);
      [epoch] = args.splice(3, 1);
    }

    if (!referenceFrame || referenceFrame.epoch == undefined) throw new TypeError('unrecognised reference frame'); // args is now lat, lon, height or latlon, height as taken by LatLonEllipsoidal .parse()
...
class Cartesian_ReferenceFrame extends Cartesian {
  ...
  convertReferenceFrame(toReferenceFrame) {
    ...
    const newTrf = toReferenceFrame; // WGS84(G730/G873/G1150) are coincident with ITRF at 10-centimetre level; WGS84(G1674) and
    ...
    const oldC = this;
    let newC = null; // is requested transformation available in single step?
  ...
  applyTransform(params, rates, δt) {
    ...
    const S = 1 + s + ṡ * δt; // apply transform (shift, scale, rotate)
