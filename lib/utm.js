...
class Utm {
  ...
  toLatLon() {
    ...
    const {
      a,
      f
    } = this.datum.ellipsoid; // WGS-84: a = 6378137, f = 1/298.257223563;
    ...
    const τ = τi;
    const φ = Math.atan(τ);
    let λ = Math.atan2(sinhηʹ, cosξʹ); // ---- convergence: Karney 2011 Eq 26, 27
    ...
    const γ = γʹ + γʺ; // ---- scale: Karney 2011 Eq 28
    ...
    const latLong = new LatLon_Utm(lat, lon, 0, this.datum); // ... and add the convergence and scale into the LatLon object ... wonderful JavaScript!
...
class LatLon_Utm extends LatLonEllipsoidal {
  ...
  toUtm(zoneOverride = undefined) {
    ...
    const latBand = mgrsLatBands.charAt(Math.floor(this.lat / 8 + 10)); // adjust zone & central meridian for Norway
    ...
    const {
      a,
      f
    } = ellipsoid; // WGS-84: a = 6378137, f = 1/298.257223563;
    ...
    let x = k0 * A * η;
    let y = k0 * A * ξ; // ---- convergence: Karney 2011 Eq 23, 24
    ...
    const γ = γʹ + γʺ; // ---- scale: Karney 2011 Eq 25
