...
class LatLonSpherical {
  ...
  distanceTo(point, radius = 6371e3) {
    ...
    if (isNaN(radius)) throw new TypeError(`invalid radius ‘${radius}’`); // a = sin²(Δφ/2) + cos(φ1)⋅cos(φ2)⋅sin²(Δλ/2)
    // δ = 2·atan2(√(a), √(1−a))
  ...
  midpointTo(point) {
    ...
    const φ2 = point.lat.toRadians();
    const Δλ = (point.lon - this.lon).toRadians(); // get cartesian coordinates for the two points
  ...
  static intersection(p1, brng1, p2, brng2) {
    ...
    if (isNaN(brng2)) throw new TypeError(`invalid brng2 ‘${brng2}’`); // see www.edwilliams.org/avform.htm#Intersection
  ...
  rhumbDistanceTo(point, radius = 6371e3) {
    ...
    const φ2 = point.lat.toRadians();
    const Δφ = φ2 - φ1;
    let Δλ = Math.abs(point.lon - this.lon).toRadians(); // if dLon over 180° take shorter rhumb line across the anti-meridian:
    ...
    const q = Math.abs(Δψ) > 10e-12 ? Δφ / Δψ : Math.cos(φ1); // distance is pythagoras on 'stretched' Mercator projection, √(Δφ² + q²·Δλ²)
  ...
  rhumbBearingTo(point) {
    ...
    const φ1 = this.lat.toRadians();
    const φ2 = point.lat.toRadians();
    let Δλ = (point.lon - this.lon).toRadians(); // if dLon over 180° take shorter rhumb line across the anti-meridian:
  ...
  rhumbDestinationPoint(distance, bearing, radius = 6371e3) {
    ...
    let φ2 = φ1 + Δφ; // check for some daft bugger going past the pole, normalise latitude if so
  ...
  static areaOf(polygon, radius = 6371e3) {
    ...
    const R = radius; // close polygon so that last point equals first point
