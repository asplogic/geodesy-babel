...
describe('latlon-nvector-spherical', function () {
  ...
  describe('dist/brng/dest fails', function () {
    ...
    test('distance (fail p)', () => should.Throw(function () {
      cambg.distanceTo({});
    }, TypeError, 'invalid point ‘[object Object]’'));
    test('distance (fail p)', () => should.Throw(function () {
    ...
    test('int.point.ch (fail)', () => should.Throw(function () {
      cambg.intermediatePointOnChordTo('paris', 0.5);
    }, TypeError, 'invalid point ‘paris’'));
  });
  ...
  describe('cross-track / along-track', function () {
    ...
    }, TypeError, 'invalid pathStart ‘null’'));
    test('cross-track (fail)', () => should.Throw(function () {
      new LatLon(10, 1).crossTrackDistanceTo(new LatLon(0, 0), 'x');
    }, TypeError, 'invalid pathBrngEnd ‘x’'));
  ...
  describe('trilaterate', function () {
    // http://gis.stackexchange.com/a/415/41129
    const p1 = new LatLon(37.418436, -121.963477),
          d1 = 265.710701754;
    const p2 = new LatLon(37.417243, -121.961889),
          d2 = 234.592423446;
    const p3 = new LatLon(37.418692, -121.960194),
          d3 = 54.8954278262;
  ...
  describe('area / enclosed (polygon-based)', function () {
    ...
    const polyOctantS = [new LatLon(-ε, ε), new LatLon(90, 0), new LatLon(-ε, 90 - ε)]; // const polyQuadrant = [ new LatLon(ε, ε), new LatLon(90, ε), new LatLon(ε, 180-ε), new LatLon(ε, 90) ];
    ...
    test('super-octant area', () => LatLon.areaOf(polyOctantS).should.equal(π * R * R / 2)); // TODO: fails: test('quadrant area', () => LatLon.areaOf(polyQuadrant).should.equal(π*R*R));
    ...
    test('hemisphere enclosed y', () => new LatLon(45, 1).isEnclosedBy(polyHemiE).should.be.true); // TODO: fails: test('hemisphere enclosed n', () => new LatLon(45, -1).isEnclosedBy(polyHemiE).should.be.false);
...
describe('navlab nvector examples (spherical)', function () {
  ...
  describe('Example 5: Surface distance', function () {
    ...
    test('dist', function () {
      dist.toFixed().should.equal('332456');
    });
  });
  describe('Example 6: Interpolated position', function () {
    ...
    test('int', function () {
      i.toString('d', 7).should.equal('89.7999805°N, 180.0000000°E');
    });
  });
  describe('Example 7: Mean position', function () {
    const points = [new LatLon(90, 0), new LatLon(60, 10), new LatLon(50, -20)];
    const mean = LatLon.meanOf(points); // 67.2362°N, 006.9175°W

    test('mean', function () {
      mean.toString().should.equal('67.2362°N, 006.9175°W');
    });
  });
  describe('Example 8: A and azimuth/distance to B', function () {
    ...
    test('destination', function () {
      b.toString().should.equal('79.9915°N, 090.0177°W');
    });
  });
  describe('Example 9: Intersection of two paths', function () {
    ...
    test('intersection', function () {
      c.toString().should.equal('40.3186°N, 055.9019°E');
    });
  });
