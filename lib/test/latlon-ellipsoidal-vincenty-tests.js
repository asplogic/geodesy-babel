...
describe('latlon-ellipsoidal-vincenty', function () {
  ...
  describe('UK', function () {
    const le = new LatLon(50.06632, -5.71475),
          jog = new LatLon(58.64402, -3.07009);
    const dist = 969954.166,
          brngInit = 9.1418775,
          brngFinal = 11.2972204;
    ...
    test('inverse distance (fail)', () => should.Throw(function () {
      le.distanceTo(null);
    }, TypeError, 'invalid point ‘null’'));
    test('inverse init brng (fail)', () => should.Throw(function () {
      le.initialBearingTo(null);
    }, TypeError, 'invalid point ‘null’'));
    test('inverse final brng (fail)', () => should.Throw(function () {
      le.finalBearingTo(null);
    }, TypeError, 'invalid point ‘null’'));
  });
  ...
  describe('surface only', function () {
    const le = new LatLon(50.06632, -5.71475, 1),
          jog = new LatLon(58.64402, -3.07009);
    test('distanceTo (fail)', () => should.Throw(function () {
      le.distanceTo(jog);
    }, RangeError, 'point must be on the surface of the ellipsoid'));
    test('initialBearingTo (fail)', () => should.Throw(function () {
      le.initialBearingTo(jog);
    }, RangeError, 'point must be on the surface of the ellipsoid'));
    test('finalBearingTo (fail)', () => should.Throw(function () {
      le.finalBearingTo(jog);
    }, RangeError, 'point must be on the surface of the ellipsoid'));
    test('destinationPoint (fail)', () => should.Throw(function () {
      le.destinationPoint(0, 0);
    }, RangeError, 'point must be on the surface of the ellipsoid'));
    test('finalBearingOn (fail)', () => should.Throw(function () {
      le.finalBearingOn(0, 0);
    }, RangeError, 'point must be on the surface of the ellipsoid'));
  });
