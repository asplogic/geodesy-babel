...
const latMin = 24;
const latMax = 66; // ---- inverse

const errors = [];

for (let lat1 = latMin; lat1 <= latMax; lat1 += 6) {
  // temperate latitudes in 6° increments
  for (let lat2 = lat1; lat2 < latMax; lat2 += 6) {
    for (let lon = -90; lon <= +90; lon += 6) {
      // hemisphere in 6° increments
...
console.info('rms inv error', (rmsErrInv * 100).toFixed(2) + '%'); // compare errors for distance below 1km?
// ---- direct (up to 100km)

errors.length = 0;

for (let lat = latMin; lat <= latMax; lat += 6) {
  // temperate latitudes in 6° increments
  for (let brng = 0; brng < 360; brng += 10) {
    // 360° of bearings in 10° increments
    const p1Sph = new LatLonS(lat, 0);
    const p1Ell = new LatLonE(lat, 0);

    for (let d = 1e3; d < 100e3; d += 1e3) {
      // 1..100km in 1km increments
