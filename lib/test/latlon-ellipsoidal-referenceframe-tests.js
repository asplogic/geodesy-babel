...
describe('latlon-ellipsoidal-referenceframe', function () {
  ...
  describe('constructor', function () {
    ...
    test('bad TRF fail', () => should.Throw(function () {
      new LatLon(0, 0, 0, null);
    }, TypeError, 'unrecognised reference frame'));
    test('bad epoch fail', () => should.Throw(function () {
      new LatLon(0, 0, 0, LatLon.referenceFrames.ITRF2014, 'xxx');
    }, TypeError, 'invalid epoch ’xxx’'));
  });
  describe('@examples', function () {
    ...
    test('parse p3', () => LatLon.parse({
      lat: 52.205,
      lon: 0.119
    }, 17, LatLon.referenceFrames.ETRF2000).toString().should.equal('52.2050°N, 000.1190°E'));
  ...
  describe('parse fail', function () {
    ...
    test('l,l bad TRF', () => should.Throw(function () {
      LatLon.parse(0, 0, 0, 0);
    }, TypeError, 'unrecognised reference frame'));
    test('l,l bad TRF', () => should.Throw(function () {
      LatLon.parse(0, 0, 0, null);
    }, TypeError, 'unrecognised reference frame'));
    test('l/l bad TRF', () => should.Throw(function () {
      LatLon.parse('0, 0', 0, 0);
    }, TypeError, 'unrecognised reference frame'));
  ...
  describe('convertReferenceFrame fail', function () {
    test('no TRF', () => should.Throw(function () {
      new LatLon(0, 0).convertReferenceFrame('ITRF2014');
    }, TypeError, 'unrecognised reference frame'));
    test('no TRF', () => should.Throw(function () {
      new Cartesian(1, 2, 3).convertReferenceFrame('ITRF2014');
    }, TypeError, 'unrecognised reference frame'));
  });
  describe('Cartesian constructor fail', function () {
    test('empty', () => should.Throw(function () {
      new Cartesian(4027893.924, 307041.993, 4919474.294, 'ITRF2000');
    }, Error, 'unrecognised reference frame'));
    test('empty', () => should.Throw(function () {
      new Cartesian(4027893.924, 307041.993, 4919474.294, LatLon.referenceFrames.ITRF2000, 'last year');
    }, Error, 'invalid epoch ’last year’'));
  });
  describe('Cartesian setter fail', function () {
    test('bad TRF', () => should.Throw(function () {
      new Cartesian(1, 2, 3).referenceFrame = 'ITRF2014';
    }, TypeError, 'unrecognised reference frame'));
    test('bad epoch', () => should.Throw(function () {
      new Cartesian(1, 2, 3).epoch = 'last year';
    }, TypeError, 'invalid epoch ’last year’'));
  });
  describe('Cartesian.toLatLon fail', function () {
    test('empty', () => should.Throw(function () {
      new Cartesian(4027893.924, 307041.993, 4919474.294).toLatLon(null);
    }, Error, 'cartesian reference frame not defined'));
  });
  ...
  describe('Dawson & Woods 2010', function () {
    // ITRF to GDA94 coordinate transformations, John Dawson and Alex Woods, Journal of Applied Geodesy 4 (2010)
    ...
    test('Appendix A geodetic', () => gda94.toString('dms', 5, 4, true).should.equal('23°40′12.44582″S, 133°53′07.84795″E +603.3361m (GDA94@2010.4559)')); // note variations in final decimal for gda94ˣ, gda94ᶻ, gda94ᵠ – difference in rounding and/or Cartesian.toLatLon()?
  ...
  describe('Proj4 Onsala observatory', function () {
    ...
    test('from GNSStrans', () => cITRF2000.convertReferenceFrame(LatLon.referenceFrames.ITRF93, 2017.0).toString(5).should.equal('[3370658.18892,711877.42369,5349787.12430](ITRF93@2017.0)')); // accurate to within 0.02mm
  });
  describe('NGS Data Sheet Meades Ranch', function () {
  ...
  describe('EUREF Permanent Network', function () {
    // epncb.oma.be/_productsservices/coord_trans (tutorial)
