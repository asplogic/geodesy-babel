...
describe('latlon-ellipsoidal', function () {
  ...
  describe('constructor', function () {
    ...
    test('non-numeric height fail', () => should.Throw(function () {
      new LatLon(0, 0, 'x');
    }, TypeError, 'invalid height ‘x’'));
  });
  describe('parse', function () {
    ...
    test('@example p3', () => LatLon.parse({
      lat: 52.205,
      lon: 0.119
    }, 17).toString().should.equal('52.2050°N, 000.1190°E'));
    test('@example p4', () => LatLon.parse({
      lat: 52.205,
      lon: 0.119,
      height: 17
    ...
    test('{ lat, lon }', () => LatLon.parse({
      lat: 51.47788,
      lon: -0.00147
    }).toString().should.equal('51.4779°N, 000.0015°W'));
    test('{ lat, lon }, h', () => LatLon.parse({
      lat: 51.47788,
      lon: -0.00147
    }, 99).toString('d', 4, 0).should.equal('51.4779°N, 000.0015°W +99m'));
    test('{ "lat", "lon" }', () => LatLon.parse({
      lat: '51.47788',
      lon: '-0.00147'
    }).toString().should.equal('51.4779°N, 000.0015°W'));
    ...
    test('{ lat, lng }', () => LatLon.parse({
      lat: 51.47788,
      lng: -0.00147
    }).toString().should.equal('51.4779°N, 000.0015°W'));
    test('{ lat, lng }, h', () => LatLon.parse({
      lat: 51.47788,
      lng: -0.00147
    }, 99).toString('d', 4, 0).should.equal('51.4779°N, 000.0015°W +99m'));
    test('{ lat, lon, h }', () => LatLon.parse({
      ...
      lon: -0.00147,
      height: 99
    }).toString('d', 4, 0).should.equal('51.4779°N, 000.0015°W +99m'));
    test('{ lat’de, long’de }', () => LatLon.parse({
      latitude: 51.47788,
      longitude: -0.00147
    }).toString().should.equal('51.4779°N, 000.0015°W'));
    test('{ lat’de, long’de }, h', () => LatLon.parse({
      latitude: 51.47788,
      longitude: -0.00147
    }, 99).toString('d', 4, 0).should.equal('51.4779°N, 000.0015°W +99m'));
    test('GeoJSON', () => LatLon.parse({
      type: 'Point',
      coordinates: [-0.00147, 51.47788]
    }).toString().should.equal('51.4779°N, 000.0015°W'));
    test('GeoJSON w/h', () => LatLon.parse({
      type: 'Point',
      coordinates: [-0.00147, 51.47788, 99]
    }).toString().should.equal('51.4779°N, 000.0015°W'));
    test('GeoJSON, h', () => LatLon.parse({
      type: 'Point',
      coordinates: [-0.00147, 51.47788]
    }, 99).toString().should.equal('51.4779°N, 000.0015°W'));
  });
  describe('parse fail', function () {
    ...
    test('single arg str', () => should.Throw(function () {
      LatLon.parse('London');
    }, TypeError, 'invalid point ‘London’'));
    test('single arg str + h', () => should.Throw(function () {
      LatLon.parse('London', 99);
    }, TypeError, 'invalid point ‘London,99’'));
    test('invalid comma arg', () => should.Throw(function () {
      LatLon.parse('London,UK');
    }, TypeError, 'invalid point ‘London,UK’'));
    test('invalid comma arg + h', () => should.Throw(function () {
      LatLon.parse('London,UK', 99);
    }, TypeError, 'invalid point ‘London,UK’'));
    test('empty object', () => should.Throw(function () {
      LatLon.parse({});
    ...
    test('invalid lat,lon', () => should.Throw(function () {
  ...
  describe('getters/setters', function () {
    ...
    test('set lat', () => {
      p.lat = 48.8584;
      p.lat.should.equal(48.8584);
    });
    test('set latitude', () => {
      p.latitude = 48.8584;
      p.latitude.should.equal(48.8584);
    });
    test('set lon', () => {
      p.lon = 2.2945;
      p.lon.should.equal(2.2945);
    });
    test('set lng', () => {
      p.lng = 2.2945;
      p.lng.should.equal(2.2945);
    });
    test('set longitude', () => {
      p.longitude = 2.2945;
      p.longitude.should.equal(2.2945);
    });
    test('set height', () => {
      p.height = 9;
      p.height.should.equal(9);
    });
    test('get ellipsoids', () => LatLon.ellipsoids.should.deep.equal({
      WGS84: {
        a: 6378137,
        b: 6356752.314245,
        f: 1 / 298.257223563
    ...
    test('set lat fail', () => should.Throw(function () {
      p.lat = 'x';
    }, TypeError, 'invalid lat ‘x’'));
    test('set latitude fail', () => should.Throw(function () {
      p.latitude = 'x';
    }, TypeError, 'invalid latitude ‘x’'));
    test('set lon fail', () => should.Throw(function () {
      p.lon = 'x';
    }, TypeError, 'invalid lon ‘x’'));
    test('set lng fail', () => should.Throw(function () {
      p.lng = 'x';
    }, TypeError, 'invalid lng ‘x’'));
    test('set longitude fail', () => should.Throw(function () {
      p.longitude = 'x';
    }, TypeError, 'invalid longitude ‘x’'));
    test('set height fail', () => should.Throw(function () {
      p.height = 'x';
    }, TypeError, 'invalid height ‘x’'));
  });
  ...
  describe('cartesian', function () {
    ...
    test('toLatLon fail (null)', () => should.Throw(function () {
      c.toLatLon(null);
    }, TypeError, 'invalid ellipsoid ‘null’'));
    test('toLatLon fail (str)', () => should.Throw(function () {
      c.toLatLon('WGS84');
    }, TypeError, 'invalid ellipsoid ‘WGS84’'));
  });
});