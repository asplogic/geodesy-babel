...
describe('latlon-nvector-ellipsoidal', function () {
  ...
  describe('deltaTo', function () {
    ...
    test('fail', () => should.Throw(function () {
      new LatLon(0, 0).deltaTo(null);
  ...
  describe('destinationPoint', function () {
    ...
    test('fail', () => should.Throw(function () {
      new LatLon(0, 0).destinationPoint(null);
    }, TypeError, 'delta is not Ned object'));
  });
});
/*
...
describe('navlab nvector examples (ellipsoidal)', function () {
  ...
  describe('Example 1: A and B to delta', function () {
    ...
    test('dist', function () {
      delta.toString(3).should.equal('[N:331730.863,E:332998.501,D:17398.304]');
    });
    test('dist', function () {
      dist.toFixed(3).should.equal('470357.384');
    });
    test('brng', function () {
      brng.toFixed(3).should.equal('45.109');
    });
    test('elev', function () {
      elev.toFixed(4).should.equal('-2.1198');
    });
  });
  describe('Example 2: B and delta to C', function () {// const n = new Nvector(1, 2, 3, 400, LatLon.datums.WGS72); // [0.267,0.535,0.802,400.000]
  ...
  describe('Example 3: ECEF-vector to geodetic latitude', function () {
    ...
    test('p', function () {
      p.toString('d', 3, 3).should.equal('39.379°N, 048.013°W +4702059.834m');
    });
  });
  describe('Example 4: Geodetic latitude to ECEF-vector', function () {
    ...
    test('c', function () {
      c.toString(3).should.equal('[6373290.277,222560.201,110568.827]');
    });
  });
});