...
describe('os-gridref', function () {
  ...
  describe('Examples', function () {
    test('Constructor', () => new Vector3d(0.267, 0.535, 0.802).should.deep.equal({
      x: 0.267,
      y: 0.535,
      z: 0.802
    }));
  ...
  describe('constructor fail', function () {
    test('texts', () => should.Throw(function () {
      new Vector3d('x', 'y', 'z');
    }, TypeError, 'invalid vector [x,y,z]'));
  });
  ...
  describe('fails', function () {
    const v123 = new Vector3d(1, 2, 3);
    const v321 = new Vector3d(3, 2, 1);
    test('plus', () => should.Throw(function () {
      v123.plus(1);
    }, TypeError, 'v is not Vector3d object'));
    test('minus', () => should.Throw(function () {
      v123.minus(1);
    }, TypeError, 'v is not Vector3d object'));
    test('times', () => should.Throw(function () {
      v123.times('x');
    }, TypeError, 'invalid scalar value ‘x’'));
    test('dividedBy', () => should.Throw(function () {
      v123.dividedBy('x');
    }, TypeError, 'invalid scalar value ‘x’'));
    test('dot', () => should.Throw(function () {
      v123.dot(1);
    }, TypeError, 'v is not Vector3d object'));
    test('cross', () => should.Throw(function () {
      v123.cross(1);
    }, TypeError, 'v is not Vector3d object'));
    test('angleTo', () => should.Throw(function () {
      v123.angleTo(1);
    }, TypeError, 'v is not Vector3d object'));
    test('angleTo', () => should.Throw(function () {
      v123.angleTo(v321, 'x');
    }, TypeError, 'n is not Vector3d object'));
    test('rotateAround', () => should.Throw(function () {
      v123.rotateAround(1);
    }, TypeError, 'axis is not Vector3d object'));
  });
});