...
describe('latlon-spherical', function () {
  ...
  describe('@examples', function () {
    ...
    test('parse p6', () => LatLon.parse({
      lat: 52.205,
      lon: 0.119
    }).toString().should.equal('52.2050°N, 000.1190°E'));
    test('parse p7', () => LatLon.parse({
      lat: '52°12′18.0″N',
      lng: '000°07′08.4″E'
    }).toString().should.equal('52.2050°N, 000.1190°E'));
    test('parse p8', () => LatLon.parse({
      type: 'Point',
  ...
  describe('constructor fail', function () {
    test('non-numeric lat fail', () => should.Throw(function () {
      new LatLon('x', 0, 0);
    }, TypeError, 'invalid lat ‘x’'));
    test('non-numeric lon fail', () => should.Throw(function () {
      new LatLon(0, 'x', 0);
    }, TypeError, 'invalid lon ‘x’'));
  });
  describe('parse fail', function () {
    test('empty', () => should.Throw(function () {
      LatLon.parse();
    }, TypeError, 'invalid (empty) point'));
    test('single arg num', () => should.Throw(function () {
      LatLon.parse(1);
    }, TypeError, 'invalid point ‘1’'));
    test('invalid comma arg', () => should.Throw(function () {
      LatLon.parse('cam', 'bridge');
    }, TypeError, 'invalid point ‘cam,bridge’'));
    test('single arg str', () => should.Throw(function () {
      LatLon.parse('cambridge');
    }, TypeError, 'invalid point ‘cambridge’'));
    test('invalid object 1', () => should.Throw(function () {
      LatLon.parse({
        y: 51.47788,
        x: -0.00147
      });
    }, TypeError, 'invalid point ‘{"y":51.47788,"x":-0.00147}’'));
    test('invalid object 2', () => should.Throw(function () {
      LatLon.parse({
        lat: 'y',
        lon: 'x'
      });
    }, TypeError, 'invalid point ‘{"lat":"y","lon":"x"}’'));
    test('null 1', () => should.Throw(function () {
      LatLon.parse(null);
    }, TypeError, 'invalid (null) point'));
    test('null 2', () => should.Throw(function () {
      LatLon.parse(null, null);
    }, TypeError, 'invalid (null) point'));
    test('null 3', () => should.Throw(function () {
      LatLon.parse(1.23, null);
    }, TypeError, 'invalid (null) point'));
  });
  ...
  describe('setters (fail)', function () {
    const camb = new LatLon(0, 0);
    test('lat', () => should.Throw(function () {
      camb.lat = 'xxx';
    }, TypeError, 'invalid lat ‘xxx’'));
    test('latitude', () => should.Throw(function () {
      camb.latitude = 'xxx';
    }, TypeError, 'invalid latitude ‘xxx’'));
    test('lon', () => should.Throw(function () {
      camb.lon = 'xxx';
    }, TypeError, 'invalid lon ‘xxx’'));
    test('lgn', () => should.Throw(function () {
      camb.lng = 'xxx';
    }, TypeError, 'invalid lng ‘xxx’'));
    test('longitude', () => should.Throw(function () {
      camb.longitude = 'xxx';
    }, TypeError, 'invalid longitude ‘xxx’'));
  });
  describe('toString', function () {
    ...
    test('bad fmt', () => should.Throw(function () {
      btTower.toString('x', 6);
    }, RangeError, 'invalid format ‘x’'));
  });
  describe('alternate point formats', function () {
    const cambg = LatLon.parse({
      lat: 52.205,
      lon: 0.119
    });
    test('distance {lat,lon}', () => cambg.distanceTo({
      lat: 48.857,
      lon: 2.351
    }).toPrecision(4).should.equal('4.043e+5'));
    test('distance {lat,lng}', () => cambg.distanceTo({
      lat: 48.857,
      lng: 2.351
    }).toPrecision(4).should.equal('4.043e+5'));
    test('distance {latitude,longitude}', () => cambg.distanceTo({
      latitude: 48.857,
      longitude: 2.351
    }).toPrecision(4).should.equal('4.043e+5'));
    test('distance GeoJSON', () => cambg.distanceTo({
      type: 'Point',
      coordinates: [2.351, 48.857]
    }).toPrecision(4).should.equal('4.043e+5'));
    test('distance {x,y} fails', () => should.Throw(function () {
      cambg.distanceTo({
        x: 48.857,
        y: 2.351
      });
    }, TypeError, 'invalid point ‘{"x":48.857,"y":2.351}’'));
    ...
    test('object (fail)', () => should.Throw(function () {
      cambg.distanceTo({
        x: 48.857,
        y: 2.351
      });
    }, TypeError, 'invalid point ‘{"x":48.857,"y":2.351}’'));
    test('string (fail)', () => should.Throw(function () {
      cambg.distanceTo('paris');
    }, TypeError, 'invalid point ‘paris’'));
  });
  describe('dist/brng/dest', function () {
    const cambg = new LatLon(52.205, 0.119),
          paris = new LatLon(48.857, 2.351);
  ...
  describe('dist/brng/dest fails', function () {
    const cambg = new LatLon(52.205, 0.119),
          paris = new LatLon(48.857, 2.351);
    test('distance (fail p)', () => should.Throw(function () {
      cambg.distanceTo({});
    }, TypeError, 'invalid point ‘{}’'));
    test('distance (fail p)', () => should.Throw(function () {
      cambg.distanceTo('paris');
    }, TypeError, 'invalid point ‘paris’'));
    test('distance (fail r)', () => should.Throw(function () {
      cambg.distanceTo(paris, 'xxx');
    }, TypeError, 'invalid radius ‘xxx’'));
    test('init brng (fail)', () => should.Throw(function () {
      cambg.initialBearingTo('paris');
    }, TypeError, 'invalid point ‘paris’'));
    test('final brng (fail)', () => should.Throw(function () {
      cambg.finalBearingTo('paris');
    }, TypeError, 'invalid point ‘paris’'));
    test('midpoint (fail)', () => should.Throw(function () {
      cambg.midpointTo('paris');
    }, TypeError, 'invalid point ‘paris’'));
    test('int.point (fail)', () => should.Throw(function () {
      cambg.intermediatePointTo('paris', 0.5);
    }, TypeError, 'invalid point ‘paris’'));
  });
  describe('intersection', function () {
    ...
    test('brng+end 1a', () => should.Throw(function () {
      LatLon.intersection(new LatLon(1, 0), new LatLon(1, 3), new LatLon(2, 2), S);
    }, TypeError, 'invalid brng1 ‘01.0000°N, 003.0000°E’'));
    test('brng+end 1b', () => should.Throw(function () {
      LatLon.intersection(new LatLon(2, 2), S, new LatLon(1, 0), new LatLon(1, 3));
    }, TypeError, 'invalid brng2 ‘01.0000°N, 003.0000°E’'));
    test('coincident', () => LatLon.intersection(new LatLon(1, 1), N, new LatLon(1, 1), E).toString().should.equal('01.0000°N, 001.0000°E'));
    const stn = new LatLon(51.8853, 0.2545),
          cdg = new LatLon(49.0034, 2.5735);
    test('stn-cdg-bxl', () => LatLon.intersection(stn, 108.547, cdg, 32.435).toString().should.equal('50.9078°N, 004.5084°E'));
    test('bad point 1', () => should.Throw(function () {
      LatLon.intersection(false, N, new LatLon(1, 0), E);
    }, TypeError, 'invalid point ‘false’'));
    test('bad point 2', () => should.Throw(function () {
      LatLon.intersection(new LatLon(0, 1), N, false, E);
    }, TypeError, 'invalid point ‘false’'));
    test('int’n (fail 1)', () => should.Throw(function () {
      LatLon.intersection(null, 'n', null, 's');
    }, TypeError, 'invalid (null) point'));
    test('int’n (fail 2)', () => should.Throw(function () {
      LatLon.intersection(stn, 'n', null, 's');
    }, TypeError, 'invalid (null) point'));
    test('int’n (fail 3)', () => should.Throw(function () {
      LatLon.intersection(stn, 'n', cdg, 's');
    }, TypeError, 'invalid brng1 ‘n’'));
    test('int’n (fail 4)', () => should.Throw(function () {
      LatLon.intersection(stn, 0, cdg, 's');
    }, TypeError, 'invalid brng2 ‘s’'));
  ...
  describe('cross-track / along-track', function () {
    ...
    const bradwell = new LatLon(53.3206, -1.7297),
          dunham = new LatLon(53.2611, -0.7972),
          partney = new LatLon(53.1887, 0.1334);
    test('cross-track', () => dunham.crossTrackDistanceTo(bradwell, partney).toPrecision(4).should.equal('-307.5'));
    test('cross-track (fail)', () => should.Throw(function () {
      new LatLon(10, 1).crossTrackDistanceTo(null, new LatLon(0, 2));
    }, TypeError, 'invalid (null) point'));
    test('cross-track (fail)', () => should.Throw(function () {
      new LatLon(10, 1).crossTrackDistanceTo(new LatLon(0, 0), null);
    }, TypeError, 'invalid (null) point'));
    ...
    test('cross-track (fail)', () => should.Throw(function () {
      new LatLon(0, 0).crossTrackDistanceTo(null, 0);
    }, TypeError, 'invalid (null) point'));
    test('cross-track (fail)', () => should.Throw(function () {
      new LatLon(0, 0).crossTrackDistanceTo(new LatLon(0, 0), 'x');
    }, TypeError, 'invalid point ‘x’'));
    test('along-track (fail)', () => should.Throw(function () {
      new LatLon(0, 0).alongTrackDistanceTo(null, 0);
    }, TypeError, 'invalid (null) point'));
    test('along-track (fail)', () => should.Throw(function () {
      new LatLon(0, 0).alongTrackDistanceTo(new LatLon(0, 0), 'x');
    }, TypeError, 'invalid point ‘x’'));
  });
  ...
  describe('rhumb lines', function () {
    const dov = new LatLon(51.127, 1.338),
          cal = new LatLon(50.964, 1.853);
    ...
    test('distance err', () => should.Throw(function () {
      dov.rhumbDistanceTo(false);
    }, TypeError, 'invalid point ‘false’'));
    ...
    test('bearing err', () => should.Throw(function () {
      dov.rhumbBearingTo(false);
    }, TypeError, 'invalid point ‘false’'));
    ...
    test('midpoint err', () => should.Throw(function () {
      dov.rhumbMidpointTo(false);
    }, TypeError, 'invalid point ‘false’'));
  });
  describe('misc', function () {
    ...
    test('equals (fail)', () => should.Throw(function () {
      new LatLon(52.205, 0.119).equals('cambg');
    }, TypeError, 'invalid point ‘cambg’'));
    test('toGeoJSON', () => new LatLon(52.205, 0.119).toGeoJSON().should.deep.equal({
      type: 'Point',
      coordinates: [0.119, 52.205]
    }));
  });
